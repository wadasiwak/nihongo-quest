...
- **中斷續做**：模擬考與單元練習進行中狀態（卷面題目、已答、題號、剩餘時間）自動存進 localStorage，關分頁不歸零；回站在首頁/級別頁看到「繼續上次」橫幅，可續做或放棄。
...
npm run e2e        # 自起 vite preview :5241，Playwright 驗關鍵行為
npm run shot       # :5242 截圖到 scripts/shots/
```
